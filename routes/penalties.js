...
router.get('/', requireAdmin, async (req, res) => {
  const penalties = (await db.query(
    `SELECT p.id, p.amount, p.date, p.type, p.event, u.username
     FROM penalties p
...
router.post('/add', requireAdmin, async (req, res) => {
  ...
  await db.query(
    'INSERT INTO penalties (user_id, type, event, amount, date) VALUES ($1, $2, $3, $4, $5)',
    [user_id, type, event, parseFloat(amount), date]
  );
